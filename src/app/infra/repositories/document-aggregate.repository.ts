...
export class DocumentAggregateDrizzleRepository extends DocumentAggregateRepository {
  ...
  loadById(
    ...
    return E.tryPromise({
      ...
      catch: (error) => new DatabaseError("Database error during aggregate load", { originalError: error })
    }).pipe(
      E.flatMap((result) =>
        result.found === false
  ...
  delete(
    ...
    return this.loadById(documentId).pipe(
      ...
      E.flatMap((opt): E.Effect<boolean, DocumentNotFoundError | BusinessRuleViolationError | DatabaseError, never> =>
        O.match(opt, {
          onNone: (): E.Effect<boolean, DocumentNotFoundError, never> =>
            E.fail(new DocumentNotFoundError(`Document not found: id=${documentId}`, "id", documentId)),
          onSome: (agg): E.Effect<boolean, BusinessRuleViolationError | DatabaseError, never> =>
            pipe(
              agg.canDelete(Boolean(options?.force)),
              E.flatMap((): E.Effect<boolean, DatabaseError, never> =>
  ...
  ): E.Effect<O.Option<DocumentId>, DocumentVersionNotFoundError | ValidationError | DatabaseError, never> {
    return pipe(
      E.tryPromise({
        try: async () => {
        ...
        catch: (error) => new DatabaseError("Database error during version lookup", { originalError: error })
      }),
      E.mapError((error): DocumentVersionNotFoundError | ValidationError | DatabaseError =>
        error instanceof DatabaseError
          ? error
          : new DatabaseError("Unexpected error during version lookup", { originalError: error })
      )
    )
  }
